...
3. 在列表页 hover 某个视频卡片后，当前网页右上角会显示当前 `Hover Preview`
4. 打开匹配的视频详情页时，扩展会将该候选信息绑定到当前详情页
5. 之后在详情页继续 hover 其他推荐视频时，只会更新 `Hover Preview`，不会覆盖当前页面绑定的候选信息
6. 如需退回旧逻辑，点击 `从本页面解析`
7. 如需移除当前详情页绑定的候选信息，点击 `清除当前页候选绑定`
8. 点击 `收藏` 后，保存的是当前 UI 中看到并可编辑的内容
